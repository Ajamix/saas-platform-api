...
export class ReviewSubmissionsController {
  ...
  create(@Body() createReviewSubmissionDto: CreateReviewSubmissionDto, @Request() req) {
    return this.reviewSubmissionsService.create(createReviewSubmissionDto, req.user.tenantId);
  }
  ...
  findAll(@Request() req,
    ...
    return this.reviewSubmissionsService.findAll(req.user.tenantId, page, limit, search);
  }
  ...
  @ApiResponse({ status: 200, type: ReviewSubmission })
  update(@Param('id') id: string, @Body() updateReviewSubmissionDto: UpdateReviewSubmissionDto, @Request() req) {
    return this.reviewSubmissionsService.update(+id, updateReviewSubmissionDto, req.user.tenantId);
  }
