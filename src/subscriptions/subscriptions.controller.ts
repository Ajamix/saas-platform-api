...
export class SubscriptionsController {
  ...
  async getActiveSubscription(@Request() req) {
    const subscription = await this.subscriptionsService.getActiveSubscription(req.user.tenantId);
  ...
  async findOne(@Param('id') id: string, @Request() req) {
    ...
    if (!req.user.isSuperAdmin && subscription.tenantId !== req.user.tenantId) {
      throw new ForbiddenException('You can only view your own tenant subscriptions');
    }
    return subscription;
  }
  ...
  async cancel(@Param('id') id: string, @Request() req) {
    ...
    if (subscription.tenantId !== req.user.tenantId) {
      throw new ForbiddenException('You can only cancel your own tenant subscription');
    }
