  ...
  async handleWebhook(@Req() req: Request, @Res() res: Response) {
    ...
    switch (event.type) {
      case 'checkout.session.completed':
        ...
        const lineItems = await this.stripeService.getSessionLineItems(session.id);
        const stripeProductId = String(lineItems.data[0]?.price?.product); 
        await this.stripeService.updateSubscriptionStatus(customerEmail, stripeProductId, subscriptionId);
        break;
